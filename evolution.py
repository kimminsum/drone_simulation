...
from drone import Drone, WIDTH, HEIGHT
...
pygame.init()
pygame.font.init()
s = pygame.display.set_mode((WIDTH, HEIGHT), 0, 32)
pygame.display.set_caption("Drone Project (Artificial Intelligence)")
    ...
    for i in range(int(N_POPULATION / (N_BEST + N_CHILDREN))):
        for bg in best_genomes:
            ...
            if random.uniform(0, 1) < PROB_MUTATION:
                new_genome.w1 += new_genome.w1 * np.random.normal(mean, stddev, size=(8, 10)) / 100 * np.random.randint(-1, 2, (8, 10))
            if random.uniform(0, 1) < PROB_MUTATION:
